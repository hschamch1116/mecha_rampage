...
- 드론 지원 아이템과 소유자별 최대 2대 동반 드론을 추가했습니다.
- 두 대의 드론은 메카 기준 좌우 편대가 기본이며, 건물·메카 안전 반경을 고려해 회피합니다.
- 드론 이동 목표점 급변 시 최대 이동량을 제한해 급가속을 줄였습니다.
- 메카 폭발 시 소유 드론도 함께 폭발·제거되고 리스폰 및 게임 리셋 시 초기화됩니다.
- 드론 아이템 본체를 밝은 청록색과 하늘색 프레임으로 변경해 식별성을 높였습니다.
- 비가 내릴 때 구름을 어둡게 하고, 비를 끄면 시간대별 맑은 구름 색상으로 복귀하도록 연동했습니다.
...
## 2026-08-05 최근 전투 이펙트 및 프리워밍 업데이트

### 모바일 전투 입력

- 모바일 조이스틱을 반투명 노란색 스타일로 조정하고 크기를 축소했습니다.
- 조이스틱 이동을 유지한 상태에서 무기 발사와 점프를 동시에 입력할 수 있습니다.
- 방향 입력 중 점프 2회 입력으로 대시가 발동하며, 모바일 버튼의 pointer-up 충돌을 수정했습니다.
- 호밍 미사일은 최대 4발을 0.2초 간격으로 발사하고, 각 탄을 위쪽 및 좌우 방향으로 랜덤 분산한 뒤 유도합니다.
- 일반 탄약과 호밍 탄약의 보유 제한을 살보 발사에도 적용했습니다.

### 전투 파괴 효과

- 바닥에 충돌한 플레이어 및 CPU 포탄에 지면 폭발과 회색 콘크리트 먼지 효과를 추가했습니다.
- 건물 붕괴 먼지 팔레트를 베이지 계열에서 중립 회색 계열로 변경했습니다.
- 가로등을 도로 양쪽에 균형 있게 배치하고 수량을 약 절반으로 줄였으며, 램프 하우징과 외곽 링 디테일을 보강했습니다.
- 폭발·탄피·파편·연기·스파크·미사일 잔상은 기존 재사용 풀 구조를 유지합니다.

### 비와 바닥 표현

- 비 시스템에 바닥 물방울 충격 링을 추가했습니다.
- 최대 96개 InstancedMesh 슬롯을 재사용하며, 비 강도 설정과 함께 표시 강도가 조절됩니다.
- 매 프레임 개별 Mesh를 생성하지 않아 비 효과의 GC 부담을 제한했습니다.

### 전투 시작 프리워밍

- 전투 로딩 단계에서 폭발, 화염, 연기, 스파크, 탄환, 레이저, 미사일 잔상, 탄피를 렌더러에 미리 컴파일합니다.
- 건물·유리·먼지 파편 배치, 발자국 링·먼지·바닥 데칼, 나무·가로등 파괴 파편, 픽업 아이템 및 비콘 디테일을 임시 렌더링해 첫 사용 지연을 줄입니다.
- 프리워밍 후 임시 오브젝트와 전용 재질은 제거하고 공용 Geometry와 실제 풀은 유지합니다.
- 일반 브라우저에서는 `window.gc()`가 제공되지 않을 수 있으므로, GC 강제 호출이 아닌 풀링과 렌더러 셰이더 컴파일 안정화를 기준으로 처리합니다.

### 검증

- `index.html` 및 `js/enemy-ai.js` 문법 검사를 통과했습니다.
- `git diff --check`를 통과했습니다.

## 2026-08-06 드론 지원 및 날씨 표현 업데이트

### 드론 아이템 및 동반 드론

- 전장에 드론 지원 아이템 2개를 배치하고, 플레이어와 CPU 모두 아이템을 획득하면 드론 동반 유닛을 생성합니다.
- 소유자별 드론은 최대 2대까지 유지하며, 두 대가 있을 때 메카 기준 좌우 편대 위치를 기본값으로 사용합니다.
- 드론이 메카와 건물 사이로 파고들지 않도록 메카 안전 반경과 건물 모서리 우회 경로를 함께 계산합니다.
- 회피 목표점이 갑자기 바뀌는 구간에는 최대 이동량을 제한해 급가속을 방지합니다.
- 메카가 폭발하면 해당 메카의 드론도 함께 폭발 이펙트를 출력하고 제거되며, 리스폰·게임 리셋 시 상태가 초기화됩니다.
- 드론 아이템 전용 복제본에는 밝은 청록색 본체와 하늘색 프레임 발광을 적용해 배경에서 쉽게 식별할 수 있도록 했습니다.

### 비와 구름 연동

- 강우량이 증가하면 구름 머티리얼이 어두운 회색 계열로 전환됩니다.
- 비를 끄면 시간대별 구름 색상과 밝기로 즉시 복귀합니다.

### 검증

- `index.html` 모듈 문법 검사 완료
- 브라우저 런타임 오류 및 경고 확인 완료
- `git diff --check` 통과
